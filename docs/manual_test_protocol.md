...
## Markdown 智能粘贴互操作（2026-09-02）

### 手动验收矩阵

| 来源 | 操作 | 预期 |
|---|---|---|
| Safari/Chrome 文章 | 复制标题、段落、粗体、列表、链接和远程图片；在 WYSIWYG 中按 Cmd+V 粘贴 | Markdown 源码包含标题、列表、样式、链接和图片语法；可见内容顺序与来源一致 |
| Word/Pages/飞书 | 复制段落、行内样式和列表；在 WYSIWYG 中按 Cmd+V 粘贴 | 优先使用 HTML；没有 HTML 时，受支持的 RTF 语义仍能保留 |
| VS Code Markdown 源码 | 复制带语法高亮的源码；在 WYSIWYG 中按 Cmd+V 粘贴 | 原始 Markdown 源码不会被二次转换 |
| Typora 渲染内容 | 复制渲染后的标题、列表和链接；按 Cmd+V 粘贴 | 语义 HTML 转换为 Markdown |
| 任意富文本来源 | 在 Markdown 源码视图和 `.txt` 中按 Cmd+V 粘贴 | 只出现纯文本；保留原始换行 |
| 任意富文本来源 | 在 WYSIWYG 中按 Cmd+Shift+V 粘贴 | 只使用 `text/plain` |
| 远程图片与嵌入图片混合内容 | 按 Cmd+V 粘贴 | 保留 HTTP(S) 图片；不保存或抓取 data/file/cid 图片 |
| 已选中文本 | 粘贴后执行撤销和重做 | 粘贴只替换一次；一次撤销恢复选区内容；一次重做重新应用粘贴 |

### Linux 环境记录

每次在 Linux 上执行矩阵前记录以下信息：

- 发行版与版本：
- 桌面环境与版本：
- 会话类型（记录 `$XDG_SESSION_TYPE`，注明 `x11` 或 `wayland`）：
