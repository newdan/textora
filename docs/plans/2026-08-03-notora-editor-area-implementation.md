...
状态：未加密编辑区已完成自动化收尾；真实加密路径按产品决定暂缓，人工 UI 验收尚未执行

范围说明：Task 7.3 不属于本轮交付，也不阻塞未加密编辑区验收；恢复加密工作时仍须先完成安全评审和独立加密引擎计划。完整计划只有在 Task 7.3 最终完成后才可标记为全部完成。
...
- [ ] 完成威胁边界和数据格式设计。
- [ ] 指定使用经过审计的加密库，禁止自行实现密码学原语。
- [ ] 给出测试向量、错误分类和密钥不可用场景。
- [ ] 评审通过后，为加密引擎单独生成细化执行计划。

**门槛：** 本编辑区计划可以先完成未加密路径和只读状态组件，但 Task 7.3 的加密创建选项必须等待加密引擎验收通过。
...
### Task 7.3：接入真实加密创建（暂缓）

**前提：** Task 0.2 派生的加密引擎计划已经全部完成并通过安全验收。

> 当前阶段仅完成加密设计，密钥服务和安全验收尚未完成；按产品决定暂缓本任务，创建面板不展示
> 加密选项，任何 typed `Encrypted` 请求均被拒绝，不创建明文文件。

**文件：** 由加密引擎计划确定，本任务自身最多修改 3 个编辑区集成文件。

- [ ] 创建面板仅在密钥服务可用时展示“加密笔记”。
- [ ] `Encrypted` 请求实际生成加密文件，不是明文文件加 catalog 标记。
- [ ] 加载、自动保存、快照、冲突和回收站路径均符合加密规格。
- [ ] 创建完成后头部只读显示“已加密”。
- [ ] 普通笔记只读显示“未加密”。
- [ ] 不存在创建后的加密切换 action。
- [ ] 运行加密计划规定的测试向量和故障恢复测试。
- [ ] 提交：`feat(notora): create encrypted notes`
