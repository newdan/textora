...
### Setext 选区范围（2026-08-19 后续优化已完成）

无选区、单光标场景仍统一在下划线之后建立可编辑块边界。带选区 Enter 已扩展为
单条多 replacement 事务：第一处删除选区，第二处在虚拟源码对应的 Setext 下划线
之后建立块边界；最终光标坐标直接使用虚拟源码增强后的坐标。这样既保留剩余标题
与下划线，也保证一次 undo 原子还原。若删除选区后已不再构成 Setext 标题，则仍
安全回落默认计划。
