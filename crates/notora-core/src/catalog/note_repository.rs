...
impl Catalog {
    ...
    pub fn upsert_active_note(&self, note: &CatalogNote) -> Result<(), CatalogError> {
        upsert_active_note_on(self.connection(), note, NoteEncryption::Unencrypted)
    }
    ...
    pub(crate) fn apply_scan_reconciliation(
        &self,
        notes: &[(CatalogNote, NoteEncryption)],
        ...
        for (note, encryption) in notes {
            upsert_active_note_on(&transaction, note, *encryption)?;
        }
        let removed_count =
...
mod tests {
    ...
    fn scan_reconciliation_rolls_back_all_notes_when_one_note_is_invalid() {
        ...
        let result = catalog.apply_scan_reconciliation(
            &[
                (valid_note, NoteEncryption::Unencrypted),
                (invalid_note, NoteEncryption::Unencrypted),
            ],
            &[],
            &[],
            &[],
