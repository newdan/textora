...
mod tests {
    ...
    #[test]
    fn new_empty() {
        let buf = GapBuffer::new(true).unwrap();
        assert_eq!(buf.len(), 0);
    }

    #[test]
    fn replace_and_read() {
        let mut buf = GapBuffer::new(true).unwrap();
        buf.replace(0..0, b"hello");
        assert_eq!(buf.len(), 5);
        assert_eq!(buf.read_forward(0), b"hello");
    }

    #[test]
    fn replace_middle() {
        let mut buf = GapBuffer::new(true).unwrap();
        buf.replace(0..0, b"hello world");
        buf.replace(5..11, b" rust");
        assert_eq!(buf.len(), 10);
        assert_eq!(buf.read_forward(0), b"hello rust");
    }
    ...
    fn read_backward() {
        let mut buf = GapBuffer::new(true).unwrap();
        buf.replace(0..0, b"hello");
        assert_eq!(buf.read_backward(3), b"hel");
        assert_eq!(buf.read_backward(0), b"");
    }

    #[test]
    fn replace_empty_range() {
        let mut buf = GapBuffer::new(true).unwrap();
        buf.replace(0..0, b"abc");
        buf.replace(1..1, b"X");
        assert_eq!(buf.len(), 4);
        // Gap buffer returns content in chunks around the gap.
        // After inserting "X" at offset 1, the gap sits between "aX" and "bc".
        let chunk1 = buf.read_forward(0);
        let chunk2 = buf.read_forward(chunk1.len());
        let mut full = Vec::new();
        full.extend_from_slice(chunk1);
        full.extend_from_slice(chunk2);
        assert_eq!(full, b"aXbc");
    }

    #[test]
    fn replace_beyond_end() {
        let mut buf = GapBuffer::new(true).unwrap();
        buf.replace(0..0, b"hi");
        // Range beyond end should be clamped
        buf.replace(1..100, b"X");
        assert_eq!(buf.read_forward(0), b"hX");
    }
}
