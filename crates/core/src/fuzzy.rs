...
mod tests {
    ...
    fn empty_haystack() {
        ...
        let (score, _) = score_fuzzy(&arena, "", "abc", true);
    ...
    fn exact_match() {
        let arena = scratch_arena(None);
        let (score, positions) = score_fuzzy(&arena, "hello", "hello", true);
        assert!(score > 0);
    ...
    fn no_match() {
        let arena = scratch_arena(None);
        let (score, _) = score_fuzzy(&arena, "hello", "xyz", true);
        assert_eq!(score, NO_MATCH);
    }
    ...
    fn partial_match() {
        let arena = scratch_arena(None);
        let (score, positions) = score_fuzzy(&arena, "hello world", "hlo", true);
        assert!(score > 0);
    ...
    fn cjk_match() {
        let arena = scratch_arena(None);
        let (score, positions) = score_fuzzy(&arena, "你好世界", "你好", true);
        assert!(score > 0);
    ...
    fn needle_longer_than_haystack() {
        let arena = scratch_arena(None);
        let (score, _) = score_fuzzy(&arena, "hi", "hello", true);
        assert_eq!(score, NO_MATCH);
    }
}
