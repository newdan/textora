...
unsafe fn lines_fwd_raw(
    ...
    return unsafe { selected_lines_fwd()(beg, end, line, line_stop) };
...
#[cfg(any(target_arch = "x86_64", target_arch = "loongarch64"))]
static LINES_FWD_DISPATCH: std::sync::OnceLock<LinesFn> = std::sync::OnceLock::new();

#[cfg(any(target_arch = "x86_64", target_arch = "loongarch64"))]
fn selected_lines_fwd() -> LinesFn {
    *LINES_FWD_DISPATCH.get_or_init(|| {
        #[cfg(target_arch = "x86_64")]
        if is_x86_feature_detected!("avx2") {
            return lines_fwd_avx2;
        }
        #[cfg(target_arch = "loongarch64")]
        if std::arch::is_loongarch_feature_detected!("lasx") {
            return lines_fwd_lasx;
        }
        #[cfg(target_arch = "loongarch64")]
        if std::arch::is_loongarch_feature_detected!("lsx") {
            return lines_fwd_lsx;
        }
        lines_fwd_fallback
    })
}
