...
unsafe fn lines_bwd_raw(
    ...
    return unsafe { selected_lines_bwd()(beg, end, line, line_stop) };
...
#[cfg(any(target_arch = "x86_64", target_arch = "loongarch64"))]
type LinesFn = unsafe fn(*const u8, *const u8, CoordType, CoordType) -> (*const u8, CoordType);

#[cfg(any(target_arch = "x86_64", target_arch = "loongarch64"))]
static LINES_BWD_DISPATCH: std::sync::OnceLock<LinesFn> = std::sync::OnceLock::new();

#[cfg(any(target_arch = "x86_64", target_arch = "loongarch64"))]
fn selected_lines_bwd() -> LinesFn {
    *LINES_BWD_DISPATCH.get_or_init(|| {
        #[cfg(target_arch = "x86_64")]
        if is_x86_feature_detected!("avx2") {
            return lines_bwd_avx2;
        }
        #[cfg(target_arch = "loongarch64")]
        if std::arch::is_loongarch_feature_detected!("lasx") {
            return lines_bwd_lasx;
        }
        #[cfg(target_arch = "loongarch64")]
        if std::arch::is_loongarch_feature_detected!("lsx") {
            return lines_bwd_lsx;
        }
        lines_bwd_fallback
    })
}
