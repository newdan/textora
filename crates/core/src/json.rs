...
impl<'a, 'i> Parser<'a, 'i> {
    ...
    fn fail(&self, pos: usize, kind: ParseErrorKind) -> ParseError {
        let mut cfg = CursorNav::new(&self.bytes);
        let pos = cfg.goto_byte(ByteIndex(pos));
        let line = pos.logical_pos.line + 1;
        let column = pos.logical_pos.unichar + 1;
