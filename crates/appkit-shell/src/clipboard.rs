...
pub(crate) fn try_write_text(text: &str) -> bool {
    ClipboardContext::new().and_then(|clipboard| clipboard.set_text(text.to_owned())).is_ok()
}
...
fn snapshot_from(source: &impl ClipboardRepresentations) -> Option<ClipboardSnapshot> {
    ...
    let raw_html = non_empty_string(source.html_text());
    let html_source_url = raw_html.as_deref().and_then(extract_cf_html_source_url);
    let html_text = raw_html.map(|html| extract_cf_html_fragment(&html).to_owned());
    let snapshot = ClipboardSnapshot {
        markdown_text,
        html_text,
        rtf_bytes: non_empty_bytes(source.rtf_bytes()),
